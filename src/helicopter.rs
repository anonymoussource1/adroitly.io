...
impl Helicopter {
	...
	pub fn update(&mut self, delta_time: &Duration, keyboard: &Keyboard, boundaries: &Vec<Boundary>) {
        ...
        for boundary in boundaries.iter() {
            if !(self.x + delta_x >= boundary.x + boundary.width ||
                self.x + delta_x + SIZE <= boundary.x ||
                self.y >= boundary.y + boundary.height ||
                self.y + SIZE <= boundary.y) {
            ...
            if !(self.x >= boundary.x + boundary.width ||
                self.x + SIZE <= boundary.x ||
                self.y + delta_y >= boundary.y + boundary.height ||
                self.y + delta_y + SIZE <= boundary.y) {
